...
- [ ] `composer check-platform-reqs --no-dev` succeeds inside the release.
...
- [ ] Persistent directories exist: `storage/app/private`, `storage/framework/cache/data`, `storage/framework/sessions`, `storage/framework/views`, and `storage/logs`.
- [ ] Persistent storage is writable by the intended PHP-FPM/deploy group, without `chmod 777`.
- [ ] Release-local `apps/api/bootstrap/cache` is writable by the intended PHP-FPM/deploy group.
...
- [ ] `CORS_ALLOWED_ORIGINS=https://laras.bakaranproject.com` and contains no wildcard.
- [ ] `SESSION_DOMAIN=laras.bakaranproject.com`.
- [ ] `SESSION_COOKIE=laras_session`.
- [ ] `SESSION_SECURE_COOKIE=true`.
- [ ] `SESSION_HTTP_ONLY=true`.
- [ ] `SESSION_SAME_SITE=lax`.
- [ ] `QUEUE_CONNECTION=sync` while no queue worker is deployed.
...
- [ ] Database and persistent-storage backup SHA-256 checksums verify.
...
- [ ] An arbitrary origin is never reflected as `Access-Control-Allow-Origin`; a fixed canonical LARAS ACAO value is acceptable for a rejected origin.
...
- [ ] Database + storage backup filenames and checksums are recorded.

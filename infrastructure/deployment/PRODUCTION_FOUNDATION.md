...
For the current S5.6 UAT, the approved source is the exact S5.6 branch HEAD recorded by PR #90. After S5.6 closes and is merged, `main` becomes the normal production release source.
...
- no queue worker is required until the application actually introduces queued work;
...
Because frontend and API are different origins, CORS is explicit and credentialed. Wildcard origins are prohibited for stateful authentication.
...
`bootstrap/cache` remains release-local and writable by the deploy/web group. Never use `chmod 777`.

## Release sequence

1. Verify the requested Git commit SHA and deployment environment.
2. Create a PostgreSQL backup before schema changes.
...
6. Ensure `storage` and `bootstrap/cache` ownership/permissions are correct.
7. Atomically move `current` to the new release.
8. Reload PHP-FPM only if required; reload Nginx only after `nginx -t` succeeds.
9. Run the smoke tests below.
10. Retain at least the previous known-good application release for rollback.

Do not run seeders automatically in production. Reference/catalog seed behavior must be explicitly reviewed before any production seeding action.
...
The API must not reflect arbitrary origins.
...
- Take a tested backup before migration-bearing releases.
